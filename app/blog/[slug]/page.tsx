...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: post ? `${post.title.rendered} | Akshay Ravikant` : 'Blog Post | Akshay Ravikant Portfolio',
    description: post?.excerpt.rendered.slice(0, 160).replace(/<[^>]+>/g, '') || 'Read this insightful blog post on my portfolio.',
    keywords: post?.title.rendered.split(' ').concat(['blog', 'Akshay', 'portfolio', 'web development']) || ['blog', 'portfolio', 'web development'],
    authors: [{ name: 'Akshay Ravikant', url: 'https://akshay.ing' }],
    openGraph: {
      title: post?.title.rendered || 'Blog Post | Akshay`s Portfolio',
      description: post?.excerpt.rendered.slice(0, 300).replace(/<[^>]+>/g, '') || 'Explore this blog post on Akshay’s portfolio.',
      url: `https://akshay.ing/blog/${resolvedParams.slug}`,
      siteName: 'Akshay’s Portfolio',
      images: [
        {
          url: post?.featuredImage?.node.mediaDetails.file
            ? `https://unitechinternet.in/wp-content/uploads/2025/02/${post.featuredImage.node.mediaDetails.file}`
            : 'https://akshay.ing/social-share-image.jpg',
          width: 1200,
          height: 630,
          alt: post?.title.rendered || 'Akshay’s Blog Post Image',
        },
    ...
    twitter: {
      card: 'summary_large_image',
      title: post?.title.rendered || 'Blog Post | Your Portfolio',
      description: post?.excerpt.rendered.slice(0, 200).replace(/<[^>]+>/g, '') || 'Read this blog post on Akshay’s portfolio.',
      images: [
        post?.featuredImage?.node.mediaDetails.file
          ? `https://unitechinternet.in/wp-content/uploads/2025/02/${post.featuredImage.node.mediaDetails.file}`
          : 'https://akshay.ing/social-share-arx.png',
      ],
  ...
  return (
    <div className="container mx-auto p-4 py-8">
      <article>
        <Script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              datePublished: post.date,
              url: `https://akshay.ing/blog/${resolvedParams.slug}`,
              image: post.featuredImage?.node.mediaDetails.file
                ? `https://unitechinternet.in/wp-content/uploads/2025/02/${post.featuredImage.node.mediaDetails.file}`
                : 'https://akshay.ing/social-share-image.jpg',
              publisher: {
                "@type": "Organization",
              ...
              mainEntityOfPage: {
                "@type": "WebPage",
                "@id": `https://akshay.ing/blog/${resolvedParams.slug}`,
              },
            }),
          }}
        />
        ...
        <p className="text-gray-600 dark:text-gray-300 mb-4 line-clamp-3 overflow-hidden">
          {post.excerpt.rendered.replace(/<[^>]+>/g, '').replace(/\n/g, '')}
        </p>
